...
threadsafe_queue<cards_send_data_t, 5> cards_send_data_queue;

void detectImageChangesTask() {
    for (;;) {
        // 传入字符串格式: cmd;index;id;card1,card2,card3,card4,card5,card6
        std::string input_card;
        std::cout << "press in the new cards here:" << std::endl;
        std::cin >> input_card;
        cards_send_data_t tmp_data = {0};
        std::string tmp_cards;
        sscanf((const char *)input_card.c_str(), "%d;%d;%s;%s",
               &tmp_data.subCmd, &tmp_data.cards_index,
        ...
        cards_send_data_queue.push(tmp_data);
        std::cout << "input_card:" << input_card
...
void changeCardsCalculateTask() {
    for (;;) {
        cards_send_data_t tmp_data = {0};
        if (cards_send_data_queue.try_pop(tmp_data)) {
